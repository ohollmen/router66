...
rapp.showchart = function (ev, act) {
  var tgtid = ev.viewtgtid || 'content';
  // Spin: 
  rapp.templated("t_wait", null, tgtid);
  var ctx;
  var initurl = act.dataurl || act.url;
  var url = act.genurl ? act.genurl(act) : initurl;
  ...
  axios.get(url).then( function (resp) {
    ...
    catch (ex) { console.log("Error getting chart graphics context: "+ex); }
    // Should be part of uisetup ???
    ...
    // NOTE: act.cmod() may make yet anaother clone. Document copts2 as cloned ... discourage cloning
